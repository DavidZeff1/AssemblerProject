...
#define ENTRY_VAR_PRINT Direct->are = 1;Direct->all = (SourceNum);combinedValue = 0;combinedValue |= Direct->are;combinedValue |= Direct->all << 2;printInBinary(combinedValue,fp);combinedValue = 0;
...
varnode* addVarNode(varnode* head, char* str,int num, int ent) { /*will add node and assign name or concatnate contents*/
    	varnode* temp = head;
